...
if str(ver) != versione:
	...
	if a == "si":
		...
		if sys.platform == "linux" or sys.platform == "linux2":
			...
			n = os.path.split(n)[-1]  
			os.system("sudo chown " + n + " opzioni.py")
		print("\n\nRiavviare il bot\n\n")
		sys.exit()
	else:
...
async def oggi(ctx):
	...
	if noggetti == 0:
		await ctx.message.channel.send(":tada: Nessuna lezione oggi")
	else:			
		await ctx.message.channel.send(lezioni)
...
async def compiti(ctx):
	...
	if lf > 3:
		with open('data/compiti.txt', 'r+') as filec:
			...
			await ctx.message.channel.send(compitistr)
	else:
		await ctx.message.channel.send(":tada: Nessun compito per domani")
...
async def verifiche(ctx):
	...
	while coggetti < noggetti:
		try:
			...
			await ctx.message.channel.send("uhm... questo non doveva succedere.")


	if nver == 0:
		await ctx.message.channel.send(":tada: Nessuna verifica questa settimana")

	elif nver == 1:
		with open('data/verifiche.txt', 'r+') as filev:
			compitistr = filev.read()
			await ctx.message.channel.send(":information_source: Questa settimana hai " + str(nver) + " verifica:\n\n" + compitistr)

	else:
		with open('data/verifiche.txt', 'r+') as filev:
			compitistr = filev.read()
			await ctx.message.channel.send(":information_source: Questa settimana hai " + str(nver) + " verifiche:\n\n" + compitistr)
